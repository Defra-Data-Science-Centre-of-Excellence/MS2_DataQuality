...
## Import Functions and Classes (dependencies) ##
...
## Note, this can be removed once integrated with the wider metadata workflow
AWS_credentials = input('Enter JSON file location where the AWS credentials live: ')
credentials = load_json_file(AWS_credentials) ## From main_aux
print(AWS_credentials)

logger = create_logger()

def generate_data_quality_output_files(bucket: str): ## List of geodataframes -- check if empty

    ## Based on an abstract based cloud (ABC)
    ## This implicitly uses the load_json_file function
    cdsm = CloudDataStorageManagerAWS(logger=logger, credentials_fp = AWS_credentials) ## From Crawler.CloudDataStorageManager.cloud_data_storage_manager
    ...
    for file in all_dataset_file:
        ...
        if dataset_file_extension=='.gpkg':
            ...
            for gdflist in gdf_list:
                ...
                Output_dataframe['Dataset'] = file["Key"]
                ...
                if len(Output_dataframe) == len(list(gdflist_df)):
                    ...
                    print("Also will need to potentially append Output_dataframe across multiple file['key'] entries")
